...
parser.add_argument('--check', type=str, default='model/ncf1')
...
class NCF(tf.keras.Model):
    def __init__(self, n_user, n_item):
        ...
        self.mlp = [tf.keras.layers.Dense(params.embedding_size, activation='relu'),
                    tf.keras.layers.Dense(params.embedding_size / 4, activation='relu'),
                    tf.keras.layers.Dense(params.embedding_size / 16)]
        self.resnet = [tf.keras.layers.Dense(params.embedding_size, activation='relu'),
                       tf.keras.layers.Dense(params.embedding_size / 4, activation='relu'),
                       tf.keras.layers.Dense(params.embedding_size / 16)]
    ...
    def __call__(self, user, item):
        ...
        gmf = tf.multiply(user_embed, item_embed)
        now = tf.concat([user_embed, item_embed], axis=-1)
        for i in range(len(self.mlp)):
            now = self.resnet[i](now) + self.mlp[i](now)

        return 5.0 * tf.squeeze(self.project(tf.concat([now, gmf], axis=-1)), axis=-1)
    ...
    def train():
        ...
        for epoch in range(1, params.epochs + 1):
            ...
            for batch, (user, item, rating) in enumerate(data_train.shuffle(buffer_size=10 * params.batch_size) \
                ...
                loss_batch = train_step(user, item, rating, model, loss_object, optimizer)
            ...
            _, loss_val = predict_step(user_val, item_val, rating_val, model, loss_object)
