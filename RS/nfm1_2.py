...
parser.add_argument('--dense_layer', type=int, default=20)
parser.add_argument('--check', type=str, default='model/nfm1_2')
...
class NFM(tf.keras.Model):
    ...
    def __call__(self, feature_index, feature_value, label):
        ...
        for i in range(params.dense_layer):
            now_afer = now_head + self.resnet_dense[i](now_before)
            now_head = now_before
            now_before = now_afer

        logits = tf.squeeze(output1 + self.out_dense(now_afer), axis=-1)
        return logits
