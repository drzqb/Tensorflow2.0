...
class NCF(tf.keras.Model):
    ...
    def __call__(self, user, item, training):
        ...
        for i in range(len(self.dropout)):
            now = self.mlp[i](self.dropout[i](now, training=training))
    ...
    def train():
        ...
        for epoch in range(1, params.epochs + 1):
            ...
            for batch, (user, item, rating) in enumerate(data_train.shuffle(buffer_size=10 * params.batch_size) \
                ...
                loss_batch = train_step(user, item, rating, ncf, loss_object, optimizer)
            ...
            _, loss_val = predict_step(user_val, item_val, rating_val, ncf, loss_object)
