...
parser.add_argument('--check', type=str, default='model/dqn1/')
parser.add_argument('--lr', type=float, default=0.0001)
parser.add_argument('--per_save', type=int, default=100)
...
def train_step(s, a, r, s_next, is_terminal, model, optim):
    with tf.GradientTape() as tape:
        q_eval = tf.gather_nd(model(s),
                              tf.concat([tf.expand_dims(tf.range(0, params.batch_size), 1), tf.expand_dims(a, 1)],
                                        axis=-1))
        q_next = model(s_next)
    ...
    def train():
        ...
        for epoch in range(1, params.epochs + 1):
            ...
            while not is_terminal:
                ...
                agent.store_transition(S, A, R, S_next, is_terminal)
