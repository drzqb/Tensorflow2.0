...
from tensorflow.keras.layers import Embedding, Dense, GRU, Flatten, Dropout
...
parser.add_argument('--check', type=str, default='model\chatbot4', help='The path where model shall be saved')
...
parser.add_argument('--drop_prob', type=float, default=0.5, help='probility for dropout')
parser.add_argument('--embedding_size', type=int, default=300, help='Embedding size for QA words')
parser.add_argument('--hidden_units', type=int, default=200, help='Hidden units for module')
...
class Encoder(Model):
    def __init__(self):
        ...
        for i in range(params.n_layers):
            ...
            self.dropout.append(Dropout(params.drop_prob))

    def __call__(self, q_embed, hidden, mask, training):
        state = tf.zeros([0, tf.shape(hidden)[1], params.hidden_units])
        now = q_embed
        for i in range(params.n_layers):
            now, state_ = self.gru[i](now, hidden[i], mask=mask)
            now = self.dropout[i](now, training=training)
...
class AttnDecoder(Model):
    ...
    def __call__(self, a_embed, q_output, hidden, q_mask, a_mask, training):
        '''
...
def train_step(q, q_length, a_input, a_target, a_length,
    ...
    with tf.GradientTape() as tape:
        ...
        q_embed = embedding(q)
        q_output, q_state = encoder(q_embed, enc_ini, mask=q_mask, training=True)
...
def predict_step(q, q_length, a_length, go,
    ...
    enc_ini = encoder.initial_hidden(batch_size)
    q_embed = embedding(q)
    q_output, q_state = encoder(q_embed, enc_ini, mask=q_mask, training=False)
    ...
    for j in range(L):
        a_embed = embedding(a_input)
        a_output_, a_state = decoder(a_embed, q_output, a_state, q_mask, a_mask[:, j:j + 1], training=False)
