...
from tensorflow.keras.layers import Embedding, Dense, GRU, Flatten, Dropout, Layer
...
parser.add_argument('--check', type=str, default='model\chatbot5', help='The path where model shall be saved')
...
class SubEncoder(Layer):
    ...
    def __call__(self, x, mask_sa, training):
        dropout1 = self.dropout1(x, training=training)
        selfattn = self.selfattn(dropout1, dropout1, mask_sa)
        addnorm1 = self.addnorm1(dropout1, selfattn)
        dropout2 = self.dropout2(addnorm1, training=training)
        feedford = self.feedford(dropout2)
        addnorm2 = self.addnorm2(dropout2, feedford)
        dropout3 = self.dropout3(addnorm2, training=training)
        return dropout3
...
class SubDecoder(Layer):
    def __init__(self):
        ...
        self.dropout4 = Dropout(params.drop_prob)

    def __call__(self, x, y, mask_sa, mask_ma, training):
        dropout1 = self.dropout1(x, training=training)
        selfattn = self.selfattn(dropout1, dropout1, mask_sa)
        addnorm1 = self.addnorm1(dropout1, selfattn)
        dropout2 = self.dropout2(addnorm1, training=training)
        mutualattn = self.mutulattn(dropout2, y, mask_ma)
        addnorm2 = self.addnorm2(dropout2, mutualattn)
        dropout3 = self.dropout3(addnorm2, training=training)
        feedford = self.feedford(dropout3)
        addnorm3 = self.addnorm3(dropout3, feedford)
        dropout4 = self.dropout4(addnorm3, training=training)
        return dropout4
...
class Decoder(Model):
    ...
    def __call__(self, a_embed, a_pos, q_output, future_mask, qa_mask, training):
        '''
        :param a_embed: (Batch, Length, Embed)
        :param q_output: (Batch, Length, Hidden)
        '''
        decoder_p = a_embed + a_pos
        for i in range(params.block):
            decoder_p = self.decoders[i](decoder_p, q_output, future_mask, qa_mask, training)
...
def train_step(q, q_length, a_input, a_target, a_length,
    ...
    with tf.GradientTape() as tape:
        ...
        encoder_output = encoder(q_embed, q_pos, mask_enc, training=True)
        ...
        logits = decoder(a_embed, a_pos, encoder_output, future_mask, mask_dec_enc, training=True)
...
def predict_step(q, q_length, a_length, go,
    ...
    encoder_output = encoder(q_embed, q_pos, mask_enc, training=False)
    ...
    for j in range(dec_L):
        logits = decoder(decoder_input, a_pos[:j + 1], encoder_output, future_mask[:, :j + 1, :j + 1],
                         mask_dec_enc[:, :j + 1], training=False)
        if j < dec_L - 1:
            decoder_input = tf.concat([decoder_input, embedding(tf.argmax(logits[:, -1:], axis=-1))], axis=1)

    decoder_output = tf.argmax(logits, axis=-1)
