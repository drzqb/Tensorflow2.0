...
class Encoder(Model):
    ...
    def __call__(self, q_embed, hidden):
        state = tf.zeros([0, tf.shape(hidden)[1], params.hidden_units])
        now = q_embed
        for i in range(params.n_layers):
            now, state_ = self.gru[i](now, hidden[i])
...
class AttnDecoder(Model):
    ...
    def __call__(self, a_embed, q_output, hidden):
        '''
        ...
        e = tf.matmul(tf.expand_dims(e, axis=1), tf.transpose(q_output, [0, 2, 1]))  # (Batch,1,Length)
        e = tf.nn.softmax(e, axis=-1)  # (Batch,1,Length)
        e = tf.matmul(e, q_output)  # (Batch,1,Hidden)
        now = tf.concat([a_embed, e], axis=-1)  # (Batch,1,Hidden+Embed)
        state = tf.zeros([0, tf.shape(hidden)[1], params.hidden_units])

        for i in range(params.n_layers):
            now, state_ = self.gru[i](now, hidden[i])  # (batch,1,hidden),(batch,hidden)
            state = tf.concat([state, tf.expand_dims(state_, axis=0)], axis=0)
...
# @tf.function
def train_step(q, a_input, a_target, a_length, embedding: ShareEmbedding, encoder: Encoder, decoder: AttnDecoder,
               loss_obj, optimizer):
    ...
    with tf.GradientTape() as tape:
        ...
        q_embed = embedding(q)
        q_output, q_state = encoder(q_embed, enc_ini)

        a_state = q_state
        a_embed = embedding(a_input)

        a_output = tf.zeros([batch_size, 0])
        for j in range(L):
            a_output_, a_state = decoder(a_embed[:, j:j + 1], q_output, a_state)
            a_output = tf.concat([a_output, tf.squeeze(a_output_, axis=1)], axis=1)
        logits = tf.reshape(a_output, [batch_size, L, -1])
        loss = loss_obj(a_target, logits)
        mask = tf.cast(tf.greater(a_input, 0), tf.float32)
...
def predict_step(q, a_length, go,
    ...
    for j in range(L):
        a_embed = embedding(a_input)
        a_output_, a_state = decoder(a_embed, q_output, a_state)
    ...
    mask = tf.sequence_mask(a_length, L, dtype=tf.int32)
    a_output *= mask
...
class USR():
    ...
    def train(self):
        ...
        test_result_batch_len = np.array([10, 12])
        ...
        for epoch in range(1, params.epochs + 1):
            ...
            for batch, (q, a_input, a_target, q_length, a_length) in enumerate(batch_data):
                loss_batch, acc_batch = train_step(q, a_input, a_target, a_length, embedding,
                ...
                result = predict_step(test_batch, test_result_batch_len, self.qa_dict['<GO>'],
                                      embedding, encoder, decoder).numpy()
                for i in range(len(test)):
                    print('Q: ', test[i])
                    print('A: ', ''.join([self.qa_reverse_dict[id] for id in result[i, :test_result_batch_len[i]]]))
                    print('')
