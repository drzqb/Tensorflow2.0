...
parser.add_argument('--check', type=str, default='model/nfm1')
...
class NFM(tf.keras.Model):
    def __init__(self, feature_size):
        ...
        self.resnet_dense = [tf.keras.layers.Dense(params.embedding_size, activation='relu') for _ in
    ...
    def __call__(self, feature_index, feature_value, label):
        ...
        for i in range(params.dense_layer):
            now1 = self.resnet_dense[2 * i](now)
            now = now + self.resnet_dense[2 * i + 1](now1)
...
class USR():
    ...
    def train():
        ...
        for epoch in range(1, params.epochs + 1):
            ...
            for batch, (index_b, value_b, label_b) in enumerate(train_dataset):
            ...
            _, auc_val = predict_step(index_val, value_val, label_val, nfm, loss_object, Auc)
            sys.stdout.write('  | auc_val: %.2f\n' % (auc_val))
            sys.stdout.flush()
